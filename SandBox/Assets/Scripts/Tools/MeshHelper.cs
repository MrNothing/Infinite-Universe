...
public class MeshHelper:MonoBehaviour
{
	void Start()
	{
		Subdivide(GetComponent<MeshFilter>().mesh, 4);
	}

	public static Hashtable IndexTriangles(Mesh mesh, float step)
	{
		...
		for (int triangle=0; triangle<mesh.triangles.Length/3; triangle++) 
		{
			...
			{
				string tris_id = flatten(barycenter, step).ToString();
			...
			//Debug.DrawLine(barycenter, barycenter*2);

			//Debug.Log("triangle_"+triangle+": "+barycenter);
		}

		return indexedTris;
	}

	public static Hashtable IndexVertices(Mesh mesh, float step)
	{
		Hashtable doubleChecker = new Hashtable ();
		Hashtable indexedVerts = new Hashtable ();
		for (int vertice=0; vertice<mesh.vertices.Length; vertice++) 
		{
			Vector3 vert = mesh.vertices[vertice];
			string vert_id = flatten(vert, step).ToString();

			if(doubleChecker[vert_id]==null)
			{

				List<int> trisInBloc;
				if(indexedVerts[vert_id]==null)
				{
					trisInBloc = new List<int>();
					trisInBloc.Add(vertice);
					indexedVerts.Add(vert_id, trisInBloc);
				}
				else
				{
					trisInBloc = (List<int>) indexedVerts[vert_id];
					trisInBloc.Add(vertice);
					indexedVerts[vert_id] = trisInBloc;
				}
				
				doubleChecker.Add(vert_id, true);
			}
			//Debug.DrawLine(barycenter, barycenter*2);
	...
	public static Vector3 flatten(Vector3 input, float step)
	{
		return new Vector3 (Mathf.Floor (input.x / step) * step, Mathf.Floor (input.y / step) * step, Mathf.Floor (input.z / step) * step);
	}
