Shader "MrNothing's Shaders/Water Advanced" { 
Properties {
	...
	_Disappear ("Disappear Distance", Float) = 100
}
...
Subshader { 
	Tags { "WaterMode"="Refractive" "RenderType"="Opaque" "RenderType"="Transparent" "Queue"="Transparent"}
	Pass {
	 ZWrite On
...
v2f vert(appdata v)
{
	...
	float ratio = distance(_WorldSpaceCameraPos, posWorld)/_Disappear;
				
	if(ratio>1)
	{
		o.alph = float2(1-ratio, 0);
		if(o.alph.x<0)
		o.alph.x=0;
	}
	return o;
